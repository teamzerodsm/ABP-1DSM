...
router.put("/me", authmiddleware, async function (req, res) {
  ...
  if (!nome || !email || !cpf) {
    return res
      .status(400)
      .json({ message: "Nome, e-mail e CPF são obrigatórios." });
  }
  ...
  if (!validarCpf(cpf)) {
    return res
      .status(400)
      .json({ message: "Digite um CPF válido. O CPF informado é inválido." });
  }
  ...
  try {
    ...
    if (novaSenha || confirmarSenha || senhaAtual) {
      ...
      if (novaSenha !== confirmarSenha) {
        return res
          .status(400)
          .json({ message: "A nova senha e a confirmação devem coincidir." });
      }
    ...
    if (e && e.code === "23505") {
      return res
        .status(409)
        .json({ message: "Já existe usuário com esses dados." });
    }
